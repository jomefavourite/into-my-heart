import * as React from 'react';
import Svg, { SvgProps, Path } from 'react-native-svg';
import { useColorScheme } from '@/hooks/useColorScheme';

const GoBack15secIcon = (props: SvgProps) => {
  const { isDarkMode } = useColorScheme();

  return (
    <Svg width={24} height={24} fill='none' {...props}>
      <Path
        stroke={isDarkMode ? '#fff' : '#303030'}
        strokeLinecap='round'
        strokeLinejoin='round'
        strokeWidth={1.5}
        d='m12 5-1.104-1.545c-.41-.576-.617-.864-.487-1.13.13-.268.46-.283 1.12-.314.156-.007.313-.011.471-.011 5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12a9.985 9.985 0 0 1 4-8'
      />
      <Path
        stroke={isDarkMode ? '#fff' : '#303030'}
        strokeLinecap='round'
        strokeWidth={1.5}
        d='M8 11c.528-.42 1.008-1.113 1.308-.984.3.128.204.552.204 1.212v4.776'
      />
      <Path
        stroke={isDarkMode ? '#fff' : '#303030'}
        strokeLinecap='round'
        strokeLinejoin='round'
        strokeWidth={1.5}
        d='M16 10h-2.64a.5.5 0 0 0-.49.402l-.366 2.102c.636-.264.957-.361 1.673-.361 1.036 0 1.927.637 1.825 1.957.018 1.56-1.242 1.92-1.825 1.9-.584-.02-1.517.2-1.677-1'
      />
    </Svg>
  );
};
export default GoBack15secIcon;
